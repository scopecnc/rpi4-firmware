...
- **Args:** Single character (e.g., `1`, `T`, `S`) OR multi-argument command string
...
- **MOVE Menu:** `!DIAG_CMD 1 X Y Z F` sets target position (e.g., `!DIAG_CMD 1 50 100 10 5`)
- **JOG Menu:** `!DIAG_CMD N DIST` jogs axis N by DIST mm (e.g., `!DIAG_CMD 1 10.5` jogs X by +10.5mm)
  - Axis numbers: 1=X, 2=Y, 3=Z, 4=F
  - Distance can be negative (e.g., `!DIAG_CMD 2 -5` jogs Y by -5mm)
  - Soft limits enforced on all axes (X≤270, Y≤150, Z≤30, F≤11.5mm)

**Example:**
```
!DIAG_CMD 1 100 50 10 0 *XX    # In MOVE menu: set target to X=100, Y=50, Z=10, F=0
!DIAG_CMD 1 10.5 *XX           # In JOG menu: jog X axis by +10.5mm
!DIAG_CMD 2 -5 *XX             # In JOG menu: jog Y axis by -5mm
```
