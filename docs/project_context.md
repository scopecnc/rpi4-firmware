...
- ✅ Soft limits enforced: X=270mm, Y=150mm, Z=30mm
...
### FOCUS Axis (Pins 15-18)
| Function | Pin | Notes |
|----------|-----|-------|
| FOCUS_STEP | 15 | Step pulse to TB6600 |
| FOCUS_DIR | 16 | Direction control |
| FOCUS_MIN | 17 | Limit switch (NO, pulled up, to GND) |
| FOCUS_MAX | 18 | Limit switch (NO, pulled up, to GND) |
...
### Limit Switch Wiring
Each limit switch:
- **One terminal** → Teensy limit pin (4, 5, 8, 9, 12, 14, 17, or 18)
- **Other terminal** → GND
...
**Firmware Version:** v1.0 (December 2025)

**Implemented Features:**
- ✅ 4-axis stepper control (X, Y, Z, Focus)
- ✅ Sequential homing with 3-phase approach (seek → back-off → precision)
...
- **Focus: 0 to 30mm** (12,000 steps) - NOT enforced per user request
...
**Homing Parameters:**
- Back-off distance: 5mm (2000 steps at 400 steps/mm)
...
**Limit Switch Noise Filtering (CRITICAL):**
- **Triple-read verification:** 200µs stable LOW required
- **Immediate interrupt disable:** `detachInterrupt()` on trigger
- **Conditional re-enable:** Only when pin returns HIGH (button released)
- Applied in all 8 ISRs (xMinISR, xMaxISR, yMinISR, yMaxISR, zMinISR, zMaxISR, fMinISR, fMaxISR)
- **Reason:** Long cables (several feet) cause 0.9V noise spikes on rising edges
- **F_MIN pin:** Moved to pin 22 (pins 17/19/20 have I2C hardware conflicts)

**Debounce Settings:**
- Limit switch debounce: 300ms (timestamp-based)
- Triple-read spacing: 100µs per read
- Prevents false triggers from cable noise
...
## 5. State Machine (v1.0 Implementation)

### States
```
IDLE          - Ready for commands, motors idle
HOMING_Z      - Homing Z axis (3-phase sequence)
...
- ERROR → (requires reset): Manual intervention needed

### Homing Sequence (3-Phase per Axis)
1. **SEEK Phase**: Fast approach toward MIN limit (800 steps/sec)
...
3. **CREEP Phase**: Slow precision approach to MIN limit (200 steps/sec)
   - Sets position to 0.00mm when limit triggered
   - Restores normal max speed
...
- **Helper functions:** `checkLimitTriggered()`, `reenableAxisInterrupts()` used everywhere
